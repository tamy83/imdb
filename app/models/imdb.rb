...
#      '10': [ '02' ]
    ...
    monthdays.each do |key, array|
      key.to_s.to_i.times { |i|
        dd = format('%02d', i+1)
        array.each { |mm|
          log("ytam posting to pool #{mm} #{dd}")
          @@pool.post do
            begin
            log("ytam posted to pool #{mm} #{dd}") 
            url = "#{DOMAIN}/search/name?birth_monthday=#{mm}-#{dd}&count=#{RESULTS_PER_PAGE}"
            @@imdb_data["#{mm}-#{dd}"] = self.scrape_search_page(url)
            log("ytam thread #{Thread.current.object_id} finished #{mm} #{dd}")
            rescue => e
              log("ytam thread #{Thread.current.object_id} error #{e.message}\n\n#{e.backtrace.join("\n")}")
            end
          end
        }
      }
...
=end
    ...
    Rails.logger.debug "ytam elasped time #{time.real}"
    @@pool.shutdown
    @@pool.wait_for_termination
    Rails.logger.debug "END IMDB SCRAPE TASK"
    Rails.logger.debug "ytam keys #{@@imdb_data.keys}"
    @@imdb_data.keys.each { |k|
      Rails.logger.debug "ytam values lenght #{@@imdb_data[k].length}"
    }
...
=end
  ...
  def self.save_person_hash(data)
